...
from optomechanics.experiment.cryolev.experiment_monitor.monitor import monitor
import time
import threading
...
class GUI:
    def long_function_wrapper(self, queue, folder, work_id, window, T_RPi, T_atto, T_zimon, T_zispectra, T_thyra):
        # This is the thread running monitor, it runs at the same time as the mainthread GUI
        try:
            show_current_value = True
            exp_mon = monitor.ExperimentMonitor(folder, show_current_value=True,
    ...
    def the_gui(self):
        ...
        column_measurement = [
            [sg.Text("4Kstage Temp (K)")],
            [sg.Text("Pressure (mbar)")],
            [sg.Text("Sample Temp (K)")],
            [sg.Text("LO power (V)")],
            [sg.Text("Trap power (V)")],
            [sg.Text("Thyra Pressure (mBar)")]

        ]

        column_value = [
            [sg.Text(size=(40, 1), key="4Kstage Temp (K)")],
            [sg.Text(size=(40, 1), key="Pressure (mbar)")],
            [sg.Text(size=(40, 1), key="Sample Temp (K)")],
            [sg.Text(size=(40, 1), key="LO power (V)")],
            [sg.Text(size=(40, 1), key="Trap power (V)")],
            [sg.Text(size=(40, 1), key="Thyra Pressure (mBar)")]
        ]
        ...
        while True:
            ...
            try:
                ...
                current_values = {"4Kstage Temp (K)": 0,
                                  "Pressure (mbar)": 0,
                                  "Sample Temp (K)": 0,
                                  "LO power (V)": 0,
                                  "Trap power (V)": 0,
                                  "Thyra Pressure (mBar)": 0}
