...
import time
import sched
from optomechanics.experiment.cryotrap_control.monitor import h5_saver
from optomechanics.experiment.cryotrap_control.drivers.zhinst_monitor import zhinst_monitor
from optomechanics.experiment.cryotrap_control.drivers.attoDRY800 import client as attoDry800Client
from optomechanics.experiment.device.daq.daq_hat.mcc_118 import daq_client
from optomechanics.experiment.cryotrap_control.drivers.Thyra import measure
import datetime
import os
import numpy as np
import queue
...
class ExperimentMonitor(object):
    ...
    attoheaders = {'pressure': 'Pressure (mbar)',
                   '4KstageTemp': '4KstageTemp (K)',
                   'SampleTemp': 'SampleTemp (K)'}
    atto_filename = 'attoDRY800_monitor.h5'

    zimon_headers = [
        'Z-PLL Demod R', 'Z-Freq (Hz)', 'X-PLL Demod R', 'X-Freq (Hz)', 'Y-PLL Demod R', 'Y-Freq (Hz)', 'Trap power', 'LO power']
    zimon_filename = 'zi_monitor_withAuxIn.h5'

    zispectra_filename = 'zispectra.h5'

    thyra_filename = 'thyra_measure.h5'
    thyra_header = 'ThyraPressure (mBar)'
    ...
    def __init__(self, folder, show_current_value=False, T_RPi=1, T_atto=1, T_zimon=5, T_zispectra=30, T_store=60, T_thyra=1,
        ...
        if T_zimon > 0:
            ...
            self.zimon.addDAQmod('Z-PLL', 'dev4968', 100,
                                 '1', ['x', 'y', 'frequency'])
            self.zimon.addDAQmod('X-PLL', 'dev4968', 100,
                                 '2', ['x', 'y', 'frequency'])
            self.zimon.addDAQmod('Y-PLL', 'dev4968', 100,
                                 '3', ['x', 'y', 'frequency'])            
    ...
    def zimon_get_datapoint(self):
        ...
        if (self.zimon is not None) and (self.T_zimon > 0):
            ...
            dat_AuxIn0 = self.zimon.getAuxIns('dev4968')[0]
            # return np.array(vals)
            dat_AuxIn1 = self.zimon.getAuxIns('dev4968')[1]
            
            
        if (dat_z is None) and (dat_x is None) and (dat_y is None) and (dat_AuxIn0 is None) and (dat_AuxIn1 is None):
            return None
        ...
        AuxIn0 = np.average(dat_AuxIn0)
        AuxIn1 = np.average(dat_AuxIn1)
        ...
        for i in range(len(self.zimon_headers)):
            dct[self.zimon_headers[i]] = (
                r_z, demod_freq_z, r_x, demod_freq_x, r_y, demod_freq_y, AuxIn0, AuxIn1)[i]
        return dct
