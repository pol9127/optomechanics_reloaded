...
HOSTNAME = 'red-pitaya-03.ee.ethz.ch'
USERNAME = 'root'
PASSWORD = 'root'
NPLL = '2'
...
class RedPitaya:
    ...
    def read_parameter_user(self, param, pll=0):
        ...
        if param == 'alpha':
            return str(self.__class__.unsigned_bitstring_to_int(self.pll_param_values[pll][param]) / (2 ** 24))
    ...
    def update_parameter_user(self, param, val, pll=0):
        ...
        if param in ['output_1', 'output_2']:
            ...
            if param == 'alpha':
                ...
                value_bitstring = '{0:024b}'.format(int(val * 2 ** 24))
...
class GUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, **kwargs):
        ...
        hostuserpass, ok = QtWidgets.QInputDialog.getText(self, "Enter Login Details","HOSTNAME,USERNAME,PASSWORD,NPLL:", QtWidgets.QLineEdit.Normal, HOSTNAME + ',' + USERNAME + ',' + PASSWORD + ',' + NPLL)
        [hostname, username, password, npll] = hostuserpass.split(',')
        if not(ok):
