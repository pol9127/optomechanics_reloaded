...
class mywindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.ui.lineEdit_3.setText('bitfiles/mexican_swimmer.bit')
    ...
    def assign_to_gpio(key):
        if key == 'DAC1' or key == 'DAC2' or key == 'led':
            return 1
        elif key == 'gain' or key == 'offset':
            return 0
        elif key == 'alpha':
            return 2
        else:
    ...
    def update_gpio1(self):
        ...
        led = int(self.table['led'])
        value = DAC2 + 2 ** 3 * DAC1 + 2 ** 6 * led
        command += str(value)
    ...
    def update_gpio2(self):
        ...
        command = '/opt/redpitaya/bin/monitor ' + gpio_address + ' '
        alpha = float(self.table['alpha'])
        value = int(2 ** 32 * alpha)
        command += str(value)
        self.ui.textEdit.append('Updating address ' + gpio_address + ' to ' + str(value) + '.')
        self.connection.sendCommand(command)
